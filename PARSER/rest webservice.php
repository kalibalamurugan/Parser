...
<?php
	...
	{ 
		...
		//echo	" $response  <br/> <br/>";
		$response = file_get_contents("https://achecker.grantsolutions.gov/checkacc.php?uri=". $url."&id=9d1a8ef5a3d348182940bbbd45c46562fa76c452&output=rest&guide=WCAG2-AA&offset=10") ;
		$xml = simplexml_load_string($response);
		// important line echo "<font size=2 color=blue> URL " . $url . "</font> <br/>";
		//  important line echo "<font size=2 color=blue> No of Errors " . $xml->summary->NumOfErrors ."</font> <br/>";
		//  important line echo "<font size=2 color=red>NumOfLikelyProblems " . $xml->summary->NumOfPotentialProblems ."</font> <br/>";
		...
		{
			//echo "Type: <font size=2 color=	#ADFF2F>" .$item->resultType ."</font><br/>";
			if (strcmp($item->resultType ,"Potential Problem")==0)
			{
				$sequenceid=explode("_", $item->sequenceID);
				$checkid[$i]=$sequenceid[2];
				//echo "<font size=3 color=red>TCheckid: ". $checkid[$i] ."</font> <br/> " ;
				//echo "<font size=2 color=green>line,col,checkid: ". $item->sequenceID."</font> <br/><br/>";
				$i++;
			}
